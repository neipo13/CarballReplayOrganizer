using NetReplayParser.CarDefinitions;
...
namespace RocketLeagueReplayParser
{
    public class Replay
    {
        ...
        public static Replay Deserialize(BinaryReader br)
        {
            ...
            //do some other stuff to put the values we care about into a list
            var actorStatesWithProperties = replay.Frames.Where(f => f.ActorStates.Any(s => s.Properties.Count > 0)).SelectMany(f => f.ActorStates.Where(s => s.Properties.Count > 0)).ToList();
            string[] clubColorStrings = { "TAGame.Team_TA:ClubColors", "TAGame.Car_TA:ClubColors" };
            var clubColorProps = actorStatesWithProperties
                .Where(s => clubColorStrings.Any(cs => s.Properties.Any(p => p.Value.PropertyName == cs)))
                .SelectMany(s => s.Properties.Where(p => clubColorStrings.Any(cs => p.Value.PropertyName == cs)))
                .Select(pair => pair.Value)
                .ToList();
            var clubColorObjects = clubColorProps.Select(p => (ClubColors)p.Data).ToList();
            replay.ClubColors = clubColorObjects;


            string[] loadoutStrings = { "TAGame.PRI_TA:ClientLoadouts" };
            var clientLoadoutProperties = actorStatesWithProperties
                .Where(s => loadoutStrings.Any(cs => s.Properties.Any(p => p.Value.PropertyName == cs)))
                .SelectMany(s => s.Properties.Where(p => loadoutStrings.Any(cs => p.Value.PropertyName == cs)))
                .Select(pair => pair.Value)
                .ToList();
            var clientLoadoutObjects = clientLoadoutProperties.SelectMany(p => ((ClientLoadouts)p.Data).LoadoutArray).ToList();
            replay.ClientLoadouts = clientLoadoutObjects;


            string[] nameStrings = { "TAGame.Default__PRI_TA", };
            var nameProps = actorStatesWithProperties
                .Where(s => loadoutStrings.Any(cs => s.Properties.Any(p => p.Value.PropertyName == cs)))
                .SelectMany(s => s.Properties.Where(p => loadoutStrings.Any(cs => p.Value.PropertyName == cs)))
                .Select(pair => pair.Value)
                .ToList();

           //Parse out the player data we care about to load in these player/loadout/id combos
            var PlayerWrappers = new List<PropertyWrapper>();
            var CarWrappers = new List<PropertyWrapper>();

            //iterate frames and place the data from stuff we care about into wrappers to store info for us
            foreach (var frame in replay.Frames)
            {
                // check for player actors
                if(frame.ActorStates.Any(a => a.Properties.Any(p => p.Value.PropertyName == PropertyNameConsts.PLAYER_NAME_ENGINE)))
                {
                    var states = frame.ActorStates.ToList();
                    foreach(var state in states)
                    {
                        var props = state.Properties;
                        var wrappers = props.Where(p => p.Value.PropertyName == PropertyNameConsts.PLAYER_NAME_ENGINE)
                ...
                //check for car actors
                if (frame.ActorStates.Any(a => a.Properties.Any(p => p.Value.PropertyName == PropertyNameConsts.CAR_INFO_ENGINE)))
                {
                    var states = frame.ActorStates.ToList();
                    foreach (var state in states)
                    {
                        var props = state.Properties;
                        var wrappers = props.Where(p => p.Value.PropertyName == PropertyNameConsts.CAR_INFO_ENGINE)
                            .Select(p => new PropertyWrapper() { Frame = frame, State = state, Property = p.Value, Type = PropertyType.Player })
                            .ToList();

                        CarWrappers.AddRange(wrappers);
                    }
                }
            }
            ...
            foreach(var playerGroup in playerNameGroups)
            {
                ...
                player.Loadout = (ClientLoadouts)(props.Where(w => w.State.Properties.Any(p => p.Value.PropertyName == PropertyNameConsts.LOADOUT_ENGINE))
                    .Select(w => w.State.Properties.First(p => p.Value.PropertyName == PropertyNameConsts.LOADOUT_ENGINE).Value)
                    .First()
                    .Data);
                player.LoadoutPaints = (ClientLoadoutsOnline)(props.Where(w => w.State.Properties.Any(p => p.Value.PropertyName == PropertyNameConsts.LOADOUT_PAINTS_ENGINE))
                    .Select(w => w.State.Properties.First(p => p.Value.PropertyName == PropertyNameConsts.LOADOUT_PAINTS_ENGINE).Value)
                    .First()
                    .Data);
                //player.UniqueID = (string)(props.Where(w => w.State.Properties.Any(p => p.Value.PropertyName == PropertyNameConsts.LOADOUT))
                //    .Select(w => w.State.Properties.First(p => p.Value.PropertyName == PropertyNameConsts.LOADOUT).Value)
                //    .First()
                //    .Data);
            ...
            //tie car info in with player info
            foreach(var w in CarWrappers)
            {
                var car = new Car();
                car.Id = w.State.Id;
                car.PlayerActorId = ((ActiveActor)w.Property.Data).ActorId;
                var teamPaint = w.State.Properties.Select(p => p.Value).FirstOrDefault(p => p.PropertyName == PropertyNameConsts.TEAM_PAINT_ENGINE);
                if(teamPaint != null)
                {
                    car.TeamPaint = (TeamPaint)teamPaint.Data;
                }
                else
                {
                    continue;
                ...
                var player = replay.Players.FirstOrDefault(p => p.ActorIDs.Any(id => id == car.PlayerActorId));
                if(player != null)
                {
                    player.AttachedCars.Add(car);
                }
                //replay.Cars.Add(car);
            }

            //tie in team/id/etc from stats fields in replay
            ...
            foreach (var stat in PlayerStats)
            {
                ...
                var name = (string)stat.FirstOrDefault(s => s.Key == PropertyNameConsts.PLAYER_NAME_PROPERTY).Value.Value;
                var team = (int)stat.FirstOrDefault(s => s.Key == PropertyNameConsts.PLAYER_TEAM_PROPERTY).Value.Value;

                var player = replay.Players.FirstOrDefault(p => p.PlayerName == name);
        ...
        public List<ClubColors> ClubColors { get; private set; }
        public List<ClientLoadout> ClientLoadouts { get; private set; }
