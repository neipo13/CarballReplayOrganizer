...
namespace RocketLeagueReplayParser.NetworkStream
{
    ...
    public class ActorState
    {
        ...
        public string ActorName
        {
            get
            {
                if (_ActorName != null) return _ActorName;
                _ActorName = "";
                foreach(var prop in this.Properties)
                {
                    var val = prop.Value;
                    if(val.PropertyName == "Engine.PlayerReplicationInfo:PlayerName")
                    {
                        _ActorName = val.Data as string;
                    }
                }
        ...
        public string UniqueID
        {
            get
            {
                ...
                {
                    var val = prop.Value;
                    if (val.PropertyName == "Engine.PlayerReplicationInfo:UniqueId")
                    {
                        _UniqueID = val.Data.ToString();
                    }
                }
        ...
        private ClientLoadouts _clientLoadouts = null;

        public ClientLoadouts ClientLoadouts
        {
            get
            {
                _clientLoadouts = new ClientLoadouts();
                foreach (var prop in this.Properties)
                {
                    var val = prop.Value;
                    if (val.PropertyName == "TAGame.PRI_TA:ClientLoadouts")
                    {
                        _clientLoadouts = val.Data as ClientLoadouts;
                    }
                }

                return _clientLoadouts;
            }
        }
        private ClientLoadoutsOnline _clientLoadoutsOnline = null;

        public ClientLoadoutsOnline ClientLoadoutsOnline
        {
            get
            {
                _clientLoadoutsOnline = new ClientLoadoutsOnline();
                foreach (var prop in this.Properties)
                {
                    var val = prop.Value;
                    if (val.PropertyName == "TAGame.PRI_TA:ClientLoadoutsOnline")
                    {
                        _clientLoadoutsOnline = val.Data as ClientLoadoutsOnline;
                    }
                }

                return _clientLoadoutsOnline;
            }
        }

        private ClientLoadoutOnline _clientLoadoutOnline = null;

        public ClientLoadoutOnline ClientLoadoutOnline
        {
            get
            {
                _clientLoadoutOnline = new ClientLoadoutOnline();
                foreach (var prop in this.Properties)
                {
                    var val = prop.Value;
                    if (val.PropertyName == "TAGame.PRI_TA:ClientLoadoutsOnline")
                    {
                        _clientLoadoutOnline = val.Data as ClientLoadoutOnline;
                    }
                }

                return _clientLoadoutOnline;
            }
        }
#endif
